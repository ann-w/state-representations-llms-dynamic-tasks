...
def main(config: DictConfig):
    ...
    if config.eval.resume_from is not None:
        ...
        summary_enabled = _as_bool(getattr(config.agent, "summary", False))
        vot_enabled = _as_bool(getattr(config.agent, "vot", False))
        vot_oracle_enabled = _as_bool(getattr(config.agent, "vot_oracle", False))
        run_name = f"{timestamp}_{config.agent.type}_{config.client.model_id.replace('/', '_')}"
        if summary_enabled:
            run_name += "_summary"
        if vot_oracle_enabled:
            run_name += "_vot_oracle"
        elif vot_enabled:
            run_name += "_vot"
