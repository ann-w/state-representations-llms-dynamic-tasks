...
## 🐉 Evaluate with local Qwen (Transformers)

You can run any of the Qwen2.5/Qwen3 VL checkpoints locally through Hugging Face Transformers. Install the required packages (GPU highly recommended):

```bash
pip install --upgrade torch transformers pillow
pip install --upgrade accelerate
pip install torchvision==0.24.1+cu128 --index-url https://download.pytorch.org/whl/cu128
```

Adjust the torchvision wheel tag (`cu128` here) to match your CUDA runtime (or drop the suffix for CPU-only installs). Pillow is needed for image inputs.

Then launch the evaluation by pointing the BALROG client to the new `qwen-local` backend:

```bash
python eval.py \
  agent.type=custom \
  agent.summary=True \
  agent.max_text_history=1000 \
  agent.max_image_history=1 \
  envs.names=babyai \
  tasks.babyai_tasks="[BabyAI-MixedTrainLocal-v0/goto]" \
  eval.num_workers=1 \
  eval.num_episodes.babyai=1 \
  client.client_name=qwen-local \
  client.model_id=Qwen/Qwen2.5-VL-3B-Instruct
```

The helper script `run_experiment_huggingface.sh` wraps the above with all paper defaults and optional toggles for summary, VOT, and image observations.
