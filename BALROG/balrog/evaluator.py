...
class Evaluator:
    ...
    def run_episode(self, task, agent, process_num=None, position=0, episode_idx=0):
        ...
        with open(csv_filename, mode="w", newline="", encoding="utf-8") as csv_file:
            ...
            csv_writer.writerow(["Step", "Action", "Reasoning", "LLMAnswer", "Observation", "Reward", "Done", "FullPrompt"])
            ...
            for step in range(max_steps_per_episode):
                ...
                full_prompt = ""
                if getattr(response, "prompt", None):
                    contents = []
                    for message in response.prompt:
                ...
                episode_log.setdefault("prompt_history", []).append(response.prompt)
                ...
                agent.prompt_builder.update_reward(reward)
                ...
                csv_writer.writerow(
                    [
                        ...
                        action,
                        reasoning,
                        getattr(response, "raw_completion", response.completion),
                        obs["text"]["long_term_context"],
                        reward,
                        done,
                        full_prompt,
                ...
                if done:
                    ...
                    if getattr(response, "reasoning", None):
                        agent.prompt_builder.update_reasoning(response.reasoning)
                    agent.prompt_builder.update_action(response.completion)
