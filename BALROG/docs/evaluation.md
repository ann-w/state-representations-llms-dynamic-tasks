...
## 🐉 Evaluate locally with Qwen (Transformers)

Install the local dependencies first (CUDA build of PyTorch recommended):
...
Replace `cu128` with the CUDA version available on your system (remove the suffix entirely for CPU-only installs).

Then point the client to the `qwen-local` backend. Images are supported automatically when `agent.max_image_history>=1`.
...
You can also reuse `run_experiment_huggingface.sh` to sweep all benchmark tasks with the same defaults used in the paper.
