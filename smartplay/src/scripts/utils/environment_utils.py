...
def initialize_environment(
    ...
    try:
        env = gym.make(
            f"smartplay:{env_name}-v0",
            include_one_shot_example=include_one_shot_example,
        )
    except TypeError:
        if include_one_shot_example:
            logging.warning(
                "Environment %s does not support one_shot_example; proceeding without it.",
                env_name,
            )
...
def initialize_model(model_name: str, model_type: str, vision: bool = False):
    query_model = get_query(model_name, model_type, vision=vision)
    logging.info(f"model: {model_name}, model_type: {model_type}, vision: {vision}")
    return query_model


def extract_summary_from_answer(answer: str) -> str:
    """Extract the concise summary returned by the model."""
    try:
        summary_match = re.search(
            r"Summary:\s*(.*?)(?:\s+Reason:|\n|$)",
            answer,
            flags=re.IGNORECASE | re.DOTALL,
        )
        if summary_match:
            summary_text = summary_match.group(1).strip()
            # Remove trailing separators that sometimes follow the summary
        ...
        return "Game progressing"
    except Exception as e:
        logging.warning(f"Error extracting summary: {e}")
