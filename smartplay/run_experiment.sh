...
# Export experiment name for Python process to consume
export EXPERIMENT_NAME="$experiment_name"

# Run the Python script in the background and redirect logs
nohup python src/scripts/main.py > "$log_file" 2>&1 &

# Get the PID of the last background process
pid=$!

echo "[INFO] Started experiment '$experiment_name' (PID $pid), logging to $log_file"
